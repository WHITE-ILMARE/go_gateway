...
func main() {
	...
	//中间件可充当业务逻辑代码
	sliceRouter.Group("/base").Use(middleware.TraceLogSliceMW(), middleware.RateLimiter(), func(c *middleware.SliceRouterContext) {
		c.Rw.Write([]byte("test func"))
	})

	//请求到反向代理
	sliceRouter.Group("/").Use(middleware.TraceLogSliceMW(), middleware.RateLimiter(), func(c *middleware.SliceRouterContext) {
