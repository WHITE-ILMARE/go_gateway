...
func main() {
	...
	if _, err := conn.Set("/test_tree2", []byte("new_content"),
		dStat.Version); err != nil { // 修改是基于版本号进行的，所以每次改之前需要查询，以获得版本号
	...
	//设置子节点
	if _, err := conn.Create("/test_tree3/subnode", []byte("node_content"),
		0, zk.WorldACL(zk.PermAll)); err != nil {
		fmt.Println("create err", err)
	}

	//获取子节点列表
	childNodes, _, err := conn.Children("/test_tree3")
	if err != nil {
