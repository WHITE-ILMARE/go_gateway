...
func (admin *ServiceController) ServiceAddTcp(c *gin.Context) {
	...
	if _, err := infoSearch.Find(lib.GORMDefaultPool, infoSearch); err == nil {
	...
	if _, err := tcpRuleSearch.Find(lib.GORMDefaultPool, tcpRuleSearch); err == nil {
	...
	if _, err := grpcRuleSearch.Find(lib.GORMDefaultPool, grpcRuleSearch); err == nil {
...
func (admin *ServiceController) ServiceUpdateTcp(c *gin.Context) {
	...
	detail, err := service.ServiceDetail(lib.GORMDefaultPool, service)
	if err != nil {
		middleware.ResponseError(c, 2002, err)
		return
	}
